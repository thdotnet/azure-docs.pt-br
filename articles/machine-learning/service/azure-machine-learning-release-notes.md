...
+ **Recursos de visualização**
  + Módulo de "Executar R script" adicionado na interface visual.

### <a name="azure-machine-learning-sdk-for-python-v1048"></a>Azure Machine Learning SDK for Python v1.0.48

+ **Novos recursos**
  + **azureml-opendatasets**
    + **azureml-contrib-opendatasets** agora está disponível como **azureml opendatasets**. O pacote antigo ainda pode funcionar, mas recomendamos que você usa **azureml opendatasets** avançando para aprimoramentos e recursos mais avançados.
    + Esse novo pacote permite que você registre abertos de conjuntos de dados como conjunto de dados no espaço de trabalho do AML e aproveitar qualquer funcionalidades que o conjunto de dados oferece.
    + Ele também inclui os recursos existentes, como consumo de conjuntos de dados abertos como quadros de dados do Pandas/SPARK e junções de local para alguns conjuntos de dados, como o clima.

+ **Recursos de visualização**
    + HyperDriveConfig agora podem aceitar o objeto de pipeline como um parâmetro para dar suporte ao ajuste de hiperparâmetro usando um pipeline.

+ **Correções de bugs e melhorias**
  + **azureml-train-automl**
    + Correção do bug sobre a perda de tipos de colunas após a transformação.
    + Correção do bug para permitir y_query ser um tipo de objeto que contém None (s) no início. 
    + Corrigido o problema no procedimento de seleção de Ensemble que desnecessariamente foi aumentando o ensemble resultante, mesmo se as pontuações permanecem constante.
    + Corrigido o problema com as configurações de whitelist_models e blacklist_models no AutoMLStep.
    + Corrigido o problema que impediu o uso de pré-processamento quando AutoML seria usado no contexto de Pipelines do ML do Azure.
  + **azureml-opendatasets**
    + Movido azureml-contrib-opendatasets para opendatasets azureml.
    + Permitidas classes de conjunto de dados aberto para ser registrado no espaço de trabalho do AML e aproveitar os recursos de conjunto de dados do AML perfeitamente.
    + NoaaIsdWeather aprimorada enriquecer o desempenho da versão de não-SPARK significativamente.
  + **azureml-explain-model**
    + Documentação online atualizada para objetos de possibilidade de interpretação.
    + Adicionado batch_size para imitar explicador quando include_local = False para streaming explicações globais em lotes para melhorar o tempo de execução de DecisionTreeExplainableModel.
    + Corrigido o problema onde `explanation.expected_values` , às vezes, retornaria um float em vez de uma lista com um float nele.
    + Adicionados valores esperados para automl de saída para imitar explicador em explicam a biblioteca de modelo.
    + Correção de importância do recurso de permutação quando o argumento de transformações fornecido para obter a importância do recurso bruto.
    + Adicionado batch_size para imitar explicador quando include_local = False para streaming explicações globais em lotes para melhorar o tempo de execução de DecisionTreeExplainableModel para a biblioteca de explainability do modelo.
  + **azureml-core**
    + Adicionada a capacidade de anexar os armazenamentos de dados DBFS na CLI do AzureML.
    + Corrigido o problema com o carregamento de repositório de dados em que uma pasta vazia será criada se `target_path` Introdução ao `/`.
    + Habilitada a comparação de dois conjuntos de dados.
    + Exclusão do modelo e a imagem agora fornece mais informações sobre a recuperação de objetos de upstream que dependem delas se exclusão falhar devido a uma dependência de upstream.
    + Preterido a configuração de RunConfiguration não utilizada no auto_prepare_environment.
  + **azureml-mlflow**
    + Melhor utilização de recursos de execuções remotas que usam azureml.mlflow.
    + A documentação do pacote do azureml mlflow aprimorada.
    + Corrigido o problema onde mlflow.log_artifacts("my_dir") economizaria artefatos sob "my_dir/artefato-caminhos" em vez de "caminhos de artefato".
  + **azureml-dataprep**
    + Objetos de fluxo de dados podem agora ser iterados produzindo uma sequência de registros.
    + Corrigido o problema em que `Dataflow.read_pandas_dataframe` falhava quando o `in_memory` argumento é definido como True.
    + Melhoria no tratamento do pandas quadros de dados com índices de coluna não cadeia de caracteres.
    + Exposto `set_diagnostics_collection()` para permitir programático habilitação e desabilitação da coleção de telemetria.
    + BottomValues e valores adicionados principais resumir.
  + **azureml-pipeline-core**
    + Parâmetro hash_paths para todas as etapas do pipeline foi preterido e será removido no futuro. Por conteúdo padrão do pasta_de_origem será transformada em hash (exceto os arquivos listados no .amlignore ou. gitignore)
    + Módulo de melhoria contínuo e ModuleStep para dar suporte a módulos específicos do tipo de computação, em preparação para a integração de RunConfiguration e realizar outras alterações para desbloquear seu uso em pipelines.
  + **azureml-pipeline-steps**
    + AzureBatchStep: Documentação aprimorada em relação a entradas/saídas.
    + AzureBatchStep: Alterado o valor padrão de delete_batch_job_after_finish como true.
  + **azureml-train-core**
    + Cadeias de caracteres agora são aceitos como destino de computação para o ajuste de Hiperparâmetro automatizada.
    + Preterido a configuração de RunConfiguration não utilizada no auto_prepare_environment.
    + Parâmetros preteridos `conda_dependencies_file_path` e `pip_requirements_file_path` em favor da `conda_dependencies_file` e `pip_requirements_file` , respectivamente.
  + **azureml-opendatasets**
    + Melhorar NoaaIsdWeather enriquecer significativamente o desempenho da versão de não-SPARK.
    
## <a name="2019-07-01"></a>2019-07-01

### <a name="azure-machine-learning-data-prep-sdk-v117"></a>SDK v1.1.7 de preparação de dados do Azure Machine Learning

Podemos revertido uma alteração que melhorou o desempenho, pois ele estava causando problemas para alguns clientes usando o Azure Databricks. Se você já teve um problema no Azure Databricks, você pode atualizar para a versão 1.1.7 usando um dos métodos a seguir:
...
+ **Novos recursos**
  + Adicionar modelo de substitutos de árvore de decisão para imitar explicador no pacote de modelo explicar azureml
  + Capacidade de especificar uma versão do CUDA para ser instalados nas imagens de inferência. Suporte para CUDA 9.0, 10.0 e o 9.1.
  + Informações sobre o treinamento de AM do Azure estão disponíveis com as imagens base [repositório do GitHub de contêineres do Azure ML](https://github.com/Azure/AzureML-Containers) e [DockerHub](https://hub.docker.com/_/microsoft-azureml)
  + CLI adicionado suporte para o agendamento de pipeline. Execute "az ml pipeline -h" para obter mais informações
  + Adicionado o parâmetro de namespace de Kubernetes personalizado à configuração de implantação de serviço Web do AKS e a CLI.
...
+ **Recursos de visualização**
    + Pontuação explainers podem, opcionalmente, salvar conda e informações de serialização mais confiável e a desserialização do pip.
    + Correção de bug para Auto seletor de recurso.
    + Mlflow.azureml.build_image atualizado para a nova api, bugs corrigidos expostos pela nova implementação.

+ **Alterações da falha**

+ **Correções de bugs e melhorias**
  + Dependência paramiko removido do azureml-core. Avisos de reprovação adicionado para o destino de computação herdados anexar métodos.
  + Melhorar o desempenho de run.create_children
  + No explicador imitar com classificação binária, corrija a ordem das probabilidades quando a probabilidade de professor é usada para dimensionar valores da forma
  + Melhor manipulação de erros e a mensagem para o aprendizado de máquina automatizado. 
  + Corrigido o problema de tempo limite de iteração automatizado para aprendizado de máquina.
  + Melhoria no desempenho de série temporal transformação automatizada para aprendizado de máquina.
...
+ **Novos recursos**
  + Adicionadas as funções de resumo para valores superiores (`SummaryFunction.TOPVALUES`) e inferior de valores (`SummaryFunction.BOTTOMVALUES`).

+ **Correções de bugs e melhorias**
  + Melhorou significativamente o desempenho de `read_pandas_dataframe`.
  + Corrigido um bug que poderia causar `get_profile()` em um fluxo de dados que aponta para arquivos binários falhe.
  + Exposto `set_diagnostics_collection()` para permitir programático habilitação e desabilitação da coleção de telemetria.
  + Alterou o comportamento da `get_profile()`. Valores NaN agora são ignoradas para média, Mín, Std e soma, que se alinha com o comportamento do Pandas.
...
+ **Novos recursos**
  + O Azure Machine Learning agora fornece suporte de primeira classe para máquina populares dados e aprendizado análise framework Scikit-learn. Usando o [ `SKLearn` estimador](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.sklearn.sklearn?view=azure-ml-py), os usuários podem facilmente treinar e implantar modelos Scikit-learn.
    + Saiba como [executar o ajuste de hiperparâmetro com Scikit-learn usando HyperDrive](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/training/train-hyperparameter-tune-deploy-with-sklearn/train-hyperparameter-tune-deploy-with-sklearn.ipynb).
  + Adicionado suporte para a criação de ModuleStep em pipelines juntamente com classes de módulo e ModuleVersion para gerenciar unidades de computação reutilizáveis.
  + Serviços Web do ACI agora dão suporte a scoring_uri persistente por meio de atualizações. O scoring_uri será alterado de IP para o FQDN. O rótulo de nome Dns para o FQDN pode ser configurado definindo o dns_name_label em deploy_configuration. 
  + Novos recursos de aprendizado de máquina automatizada:
    + Featurizer STL para previsão
    + Clustering KMeans está habilitado para a limpeza de recurso
  + Aprovações de cota AmlCompute apenas se tornou mais rápidas! Agora podemos ter automatizou o processo para aprovar suas solicitações dentro de um limite de cota. Para obter mais informações sobre como funcionam as cotas, saiba [como gerenciar cotas](https://docs.microsoft.com/azure/machine-learning/service/how-to-manage-quotas).

+ **Recursos de visualização**
    + Integração com o [MLflow](https://mlflow.org) 1.0.0 controle por meio de pacote do azureml mlflow ([notebooks de exemplo](https://aka.ms/azureml-mlflow-examples)).
    + Envie anotações do Jupyter como uma execução. [Documentação de referência de API](https://docs.microsoft.com/python/api/azureml-contrib-notebook/azureml.contrib.notebook?view=azure-ml-py)
    + Visualização pública da [dados descompasso Detector](https://docs.microsoft.com/python/api/azureml-contrib-datadrift/azureml.contrib.datadrift?view=azure-ml-py) por meio de pacote azureml-contrib-datadrift ([notebooks de exemplo](https://aka.ms/azureml-datadrift-example)). Descompasso de dados é uma das principais razões a onde a precisão do modelo degrada ao longo do tempo. Isso acontece quando os dados fornecidos modelar em produção é diferente do que o modelo foi treinado nos dados. Detector de descompasso de dados do AML ajuda o cliente para monitorar o descompasso de dados e envia o alerta sempre que o descompasso for detectado. 

+ **Alterações da falha**

+ **Correções de bugs e melhorias**
  + RunConfiguration carregar e salvar dá suporte à especificação de um caminho de arquivo completo com compatibilidade com back completa para o comportamento anterior.
  + Adicionado o armazenamento em cache em ServicePrincipalAuthentication, desativado por padrão.
  + Habilite o log de vários gráficos sob o mesmo nome de métrica.
  + Agora corretamente podem ser importadas do azureml.core de classe de modelo (`from azureml.core import Model`).
  + Nas etapas de pipeline, `hash_path` parâmetro foi preterido. Novo comportamento é pasta_de_origem completa, exceto os arquivos listados no .amlignore ou. gitignore de hash.
  + Nos pacotes de pipeline, vários `get_all` e `get_all_*` métodos foram preteridos em favor da `list` e `list_*`, respectivamente.
  + azureml.Core.get_run não requer mais classes a serem importados antes de retornar o tipo de execução original.
  + Corrigido um problema em que algumas chamadas para a atualização do serviço Web não foram disparada para uma atualização.
  + Tempo limite de pontuação em AKS webservices deve estar entre 5 ms e 300000ms. Máximo permitido scoring_timeout_ms para solicitações de pontuação tem sido aumentado de 1 minuto para 5 minutos.
  + Agora tem objetos LocalWebservice `scoring_uri` e `swagger_uri` propriedades.
  + Movido a criação do diretório de saídas e saídas de upload de diretório do processo de usuário. Habilitado o histórico de execução do SDK para executar em cada processo de usuário. Isso deve resolver alguns problemas de sincronização pelos treinamento distribuído é executado.
  + O nome do log azureml gravado do nome do processo de usuário agora incluirá o nome do processo (distribuídas somente para treinamento) e o PID.

### <a name="azure-machine-learning-data-prep-sdk-v115"></a>SDK v1.1.5 de preparação de dados do Azure Machine Learning

+ **Correções de bugs e melhorias**
  + Para valores de datetime interpretado que têm um formato de ano de 2 dígitos, o intervalo de anos válidos foi atualizado para corresponder à versão podem do Windows. O intervalo foi alterado de 1930 2029 para 1950-2049.
  + Durante a leitura em um arquivo e a configuração `handleQuotedLineBreaks=True`, `\r` será tratado como uma nova linha.
  + Corrigido um bug que causou `read_pandas_dataframe` falha em alguns casos.
