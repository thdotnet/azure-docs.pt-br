...
XFSv5 | Recursos XFSv5 em sistemas de arquivos XFS, como a soma de verificação dos metadados, têm suporte do Serviço de Mobilidade versão 9.10 em diante. Use o utilitário xfs_info para verificar o superbloco XFS da partição. Se `ftype` é definido como 1, em seguida, recursos de XFSv5 estão em uso.
BTRFS |Da versão 9.22, BTRFS tem suporte, exceto para os seguintes cenários</br>Se o volume de subpropriedades de sistema de arquivos BTRFS for alterado depois de habilitar a proteção, não há suporte para BTRFS. </br>Se o sistema de arquivos BTRFS é distribuído entre vários discos, BTRFS não é suportada.</br>Se o sistema de arquivos BTRFS dá suporte a RAID, não há suporte para BTRFS.
