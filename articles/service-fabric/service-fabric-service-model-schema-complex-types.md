...
Limites de memória em MB. Deve ser um inteiro positivo. Os pacotes de códigos (contêineres ou processos) não podem alocar mais memória do que esse limite. A tentativa de fazer isso resultará em uma exceção de memória insuficiente.
...
O limite flexível (em MB) para governança de memória que é imposto apenas quando a contenção de memória é detectada no nó. Deve ser um inteiro positivo.
...
Taxa de E/S máxima (leitura e gravação) em termos de IOps que pode ser usada. Deve ser um inteiro positivo.
...
Limites de memória do kernel em bytes.  Deve ser um inteiro positivo.  Observe que isso é específico do Linux, o docker no Windows apresentará um erro se isso estiver definido.
...
Tamanho de /dev/shm em bytes. Se omitido, o sistema usará 64MB.  Deve ser um inteiro positivo.  Observe que isso é específico do Linux, no entanto, NÃO ocorrerá erro no docker se estiver especificado.  É simplesmente ignorado.
