...
|TotalConnectionFailures|Falhas de conexão totais|Contagem|Average|Total de falhas em tentativas de conexão.|ServerResourceType|
...
|ProcessingPoolJobQueueLength|Comprimento da fila de trabalho do pool de processamento|Count|Average|Número de trabalhos não de E/S na fila do pool de threads de processamento.|ServerResourceType|
...
|QuotaBlocked|Memória: Cota bloqueada|Count|Average|Número atual de solicitações de cota bloqueadas até que outras cotas de memória sejam liberadas.|ServerResourceType|
...
|ProcessingPoolIdleIOJobThreads|Threads: Threads de trabalho de E/S ociosos do pool de processamento|Count|Average|Número de threads ociosos para trabalhos de E/S no pool de threads de processamento.|ServerResourceType|
...
|ShortParsingIdleThreads|Threads: Threads ociosos de análise resumida|Contagem|Average|Número de threads ociosos no pool de threads de análise resumida.|ServerResourceType|
|ShortParsingJobQueueLength|Threads: Tamanho da fila de trabalhos de análise resumida|Contagem|Average|Número de trabalhos na fila do pool de threads de análise resumida.|ServerResourceType|
...
|TotalRequests|Total de Solicitações de Gateway|Contagem|Total|O número total de solicitações de gateway em um determinado período. Essa métrica foi preterida, é recomendável usar a `Requests` nova métrica. |Local, Nome do host|
|SuccessfulRequests|Solicitações de Gateway com Êxito|Count|Total|O número total de solicitações de gateway bem-sucedidas em um determinado período. Essa métrica foi preterida, é recomendável usar a `Requests` nova métrica.|Local, Nome do host|
|UnauthorizedRequests|Solicitações de Gateway não autorizado|Count|Total| O número total de solicitações de gateway não autorizadas em um determinado período. Essa métrica foi preterida, é recomendável usar a `Requests` nova métrica.|Local, Nome do host|
|FailedRequests|Solicitações de Gateway com Falha|Count|Total|O número total de solicitações de gateway com falha em um determinado período. Essa métrica foi preterida, é recomendável usar a `Requests` nova métrica.|Local, Nome do host|
...
|getcommands|Obtenções|Contagem|Total||ShardId|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|Porcentagem de CPU|Porcentagem de CPU|Porcentagem|Average|O percentual de unidades de computação alocadas que estão atualmente em uso pelas Máquinas Virtuais.|Sem dimensões|
...
|Bytes de Leitura no Disco/s|Leitura de Disco|BytesPerSecond|Average|Média de bytes lidos do disco durante o período de monitoramento.|Sem dimensões|
|Bytes de Gravação no Disco/s|Gravação de disco|BytesPerSecond|Average|Média de bytes gravados no disco durante o período de monitoramento.|Sem dimensões|
|Operações de Leitura de Disco por segundo|Operações de leitura de disco/Seg|CountPerSecond|Average|IOPS de Leitura do Disco.|Sem dimensões|
|Operações/s de Gravação do Disco|Operações de gravação de disco/Seg|CountPerSecond|Average|IOPS de Gravação no Disco.|Sem dimensões|

## <a name="microsoftclassiccomputedomainnamesslotsroles"></a>Microsoft.ClassicCompute/domainNames/slots/roles

|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|Porcentagem de CPU|Porcentagem de CPU|Porcentagem|Average|O percentual de unidades de computação alocadas que estão atualmente em uso pelas Máquinas Virtuais.|RoleInstanceId|
...
|Bytes de Leitura no Disco/s|Leitura de Disco|BytesPerSecond|Average|Média de bytes lidos do disco durante o período de monitoramento.|RoleInstanceId|
|Bytes de Gravação no Disco/s|Gravação de disco|BytesPerSecond|Average|Média de bytes gravados no disco durante o período de monitoramento.|RoleInstanceId|
|Operações de Leitura de Disco por segundo|Operações de leitura de disco/Seg|CountPerSecond|Average|IOPS de Leitura do Disco.|RoleInstanceId|
|Operações/s de Gravação do Disco|Operações de gravação de disco/Seg|CountPerSecond|Average|IOPS de Gravação no Disco.|RoleInstanceId|
...
|BlockedCalls|Chamadas Bloqueadas|Contagem|Total|Número de chamadas que excederam o limite de taxa ou de cota.|ApiName, OperationName, região|
...
|CharactersTranslated|Caracteres Traduzidos|Contagem|Total|Número total de caracteres na solicitação de texto recebida.|ApiName, OperationName, região|
...
|Operações de Leitura de Disco por segundo|Operações de leitura de disco/Seg|CountPerSecond|Average|IOPS de Leitura de Disco|Sem dimensões|
|Operações/s de Gravação do Disco|Operações de gravação de disco/Seg|CountPerSecond|Average|IOPS de Gravação do Disco|Sem dimensões|
|Créditos de CPU Restantes|Créditos de CPU restantes|Contagem|Average|Número total de créditos disponíveis para disparo contínuo|Sem dimensões|
|Créditos de CPU consumidos|Créditos de CPU consumidos|Count|Average|Número total de créditos consumidos pela Máquina Virtual|Sem dimensões|
|Bytes de Leitura Por Disco/s|Bytes de Leitura do Disco de Dados/s (Preteridos)|CountPerSecond|Average|Leitura de Bytes/s de um único disco durante o período de monitoramento|SlotId|
|Bytes de Gravação Por Disco/s|Bytes de Gravação do Disco de Dados/s (Preteridos)|CountPerSecond|Average|Bytes/s gravados em um único disco durante o período de monitoramento|SlotId|
|Operações de Leitura Por Disco/s|Operações de Leitura do Disco de Dados/s (Preteridas)|CountPerSecond|Average|Ler IOPS de um único disco durante o período de monitoramento|SlotId|
|Operações de Gravação Por Disco/s|Operações de Gravação do Disco de Dados/s (Preteridas)|CountPerSecond|Average|Gravar IOPS de um único disco durante o período de monitoramento|SlotId|
|QD Por Disco|QD do Disco de Dados (Preterido)|Count|Average|Profundidade da Fila do Disco de Dados (ou Tamanho da Fila)|SlotId|
|Bytes de Leitura do SO Por Disco/s|Bytes de Leitura do Disco do SO/s (Preteridos)|CountPerSecond|Average|Leitura de Bytes/s de um único disco durante o período de monitoramento do disco do SO|Sem dimensões|
|Bytes de Gravação do SO Por Disco/s|Bytes de Gravação do Disco do SO/s (Preteridos)|CountPerSecond|Average|Bytes/s gravados em um único disco durante o período de monitoramento do disco do SO|Sem dimensões|
|Operações de Leitura do SO Por Disco/s|Operações de Leitura do Disco do SO/s (Preteridas)|CountPerSecond|Average|Ler IOPS de um único disco durante o período de monitoramento para disco do SO|Sem dimensões|
|Operações de Gravação do SO Por Disco/s|Operações de Gravação do Disco do SO/s (Preteridos)|CountPerSecond|Average|Gravar IOPS de um único disco durante o período de monitoramento para disco do SO|Sem dimensões|
|QD do SO Por Disco|QD de Disco do SO (Preterido)|Count|Average|Profundidade da Fila do Disco do SO (ou Comprimento da Fila)|Sem dimensões|
|Bytes de Leitura do Disco de Dados/s|Bytes de Leitura do Disco de Dados/s (Versão Prévia)|CountPerSecond|Average|Leitura de Bytes/s de um único disco durante o período de monitoramento|LUN|
|Bytes de Gravação do Disco de Dados/s|Bytes de Gravação do Disco de Dados/s (Versão Prévia)|CountPerSecond|Average|Bytes/s gravados em um único disco durante o período de monitoramento|LUN|
|Operações de Leitura do Disco de Dados/s|Operações de Leitura do Disco de Dados/s (Versão Prévia)|CountPerSecond|Average|Ler IOPS de um único disco durante o período de monitoramento|LUN|
|Operações de Gravação do Disco de Dados/s|Operações de Gravação do Disco de Dados/s (Versão Prévia)|CountPerSecond|Average|Gravar IOPS de um único disco durante o período de monitoramento|LUN|
|Profundidade da Fila de Disco de Dados|Profundidade da Fila de Disco de Dados (Versão prévia)|Count|Average|Profundidade da Fila do Disco de Dados (ou Tamanho da Fila)|LUN|
|Bytes de Leitura do Disco do SO/s|Bytes de Leitura do Disco do SO/s (Versão Prévia)|CountPerSecond|Average|Leitura de Bytes/s de um único disco durante o período de monitoramento do disco do SO|Sem dimensões|
|Bytes de Gravação do Disco do SO/s|Bytes de Gravação do Disco do SO/s (Versão Prévia)|CountPerSecond|Average|Bytes/s gravados em um único disco durante o período de monitoramento do disco do SO|Sem dimensões|
|Operações de Leitura do Disco do SO/s|Operações de Leitura do Disco do SO/s (Versão Prévia)|CountPerSecond|Average|Ler IOPS de um único disco durante o período de monitoramento para disco do SO|Sem dimensões|
|Operações de Gravação do Disco do SO/s|Operações de Gravação do Disco do SO/s (Versão Prévia)|CountPerSecond|Average|Gravar IOPS de um único disco durante o período de monitoramento para disco do SO|Sem dimensões|
|Profundidade da Fila de Disco do SO|Profundidade da Fila de Disco do SO (Versão prévia)|Count|Average|Profundidade da Fila do Disco do SO (ou Comprimento da Fila)|Sem dimensões|
|Fluxos de Entrada|Fluxos de Entrada (Versão prévia)|Contagem|Average|Fluxos de Entrada serão o número de fluxos atuais na direção de entrada (tráfego para dentro da VM)|Sem dimensões|
|Fluxos de Saída|Fluxos de Saída (Versão prévia)|Contagem|Average|Fluxos de Saída serão o número de fluxos atuais na direção de saída (tráfego para fora da VM)|Sem dimensões|
|Taxa Máxima de Criação de Fluxos de Entrada|Taxa Máxima de Criação de Fluxos de Entrada (Versão Prévia)|CountPerSecond|Average|A taxa máxima de criação de fluxos de entrada (o tráfego que entra na VM)|Sem dimensões|
|Taxa Máxima de Criação de Fluxos de Saída|Taxa Máxima de Criação de Fluxos de Saída (Versão Prévia)|CountPerSecond|Average|A taxa máxima de criação de fluxos de saída (o tráfego que sai da VM)|Sem dimensões|
|Acerto de Leitura do Cache de Disco de Dados Premium|Acerto de Leitura do Cache de Disco de Dados Premium (Versão prévia)|Percent|Average|Acerto de Leitura do Cache de Disco de Dados Premium|LUN|
|Erro de Leitura do Cache de Disco de Dados Premium|Erro de Leitura do Cache de Disco de Dados Premium (Versão prévia)|Percent|Average|Erro de Leitura do Cache de Disco de Dados Premium|LUN|
|Acerto de Leitura do Cache de Disco do SO Premium|Acerto de Leitura do Cache de Disco do SO Premium (Versão Prévia)|Percent|Average|Acerto de Leitura do Cache de Disco do SO Premium|Sem dimensões|
|Erro de Leitura do Cache de Disco de SO Premium|Erro de Leitura do Cache de Disco de SO Premium (Versão prévia)|Percent|Average|Erro de Leitura do Cache de Disco de SO Premium|Sem dimensões|
...
|Operações de Leitura de Disco por segundo|Operações de leitura de disco/Seg|CountPerSecond|Average|IOPS de Leitura de Disco|VMName|
|Operações/s de Gravação do Disco|Operações de gravação de disco/Seg|CountPerSecond|Average|IOPS de Gravação do Disco|VMName|
|Créditos de CPU restantes|Créditos de CPU restantes|Count|Average|Número total de créditos disponíveis para disparo contínuo|Sem dimensões|
|Créditos de CPU consumidos|Créditos de CPU consumidos|Count|Average|Número total de créditos consumidos pela Máquina Virtual|Sem dimensões|
|Bytes de Leitura Por Disco/s|Bytes de Leitura do Disco de Dados/s (Preteridos)|CountPerSecond|Average|Leitura de Bytes/s de um único disco durante o período de monitoramento|SlotId|
|Bytes de Gravação Por Disco/s|Bytes de Gravação do Disco de Dados/s (Preteridos)|CountPerSecond|Average|Bytes/s gravados em um único disco durante o período de monitoramento|SlotId|
|Operações de Leitura Por Disco/s|Operações de Leitura do Disco de Dados/s (Preteridas)|CountPerSecond|Average|Ler IOPS de um único disco durante o período de monitoramento|SlotId|
|Operações de Gravação Por Disco/s|Operações de Gravação do Disco de Dados/s (Preteridas)|CountPerSecond|Average|Gravar IOPS de um único disco durante o período de monitoramento|SlotId|
|QD Por Disco|QD do Disco de Dados (Preterido)|Contagem|Average|Profundidade da Fila do Disco de Dados (ou Tamanho da Fila)|SlotId|
|Bytes de Leitura do SO Por Disco/s|Bytes de Leitura do Disco do SO/s (Preteridos)|CountPerSecond|Average|Leitura de Bytes/s de um único disco durante o período de monitoramento do disco do SO|Sem dimensões|
|Bytes de Gravação do SO Por Disco/s|Bytes de Gravação do Disco do SO/s (Preteridos)|CountPerSecond|Average|Bytes/s gravados em um único disco durante o período de monitoramento do disco do SO|Sem dimensões|
|Operações de Leitura do SO Por Disco/s|Operações de Leitura do Disco do SO/s (Preteridas)|CountPerSecond|Average|Ler IOPS de um único disco durante o período de monitoramento para disco do SO|Sem dimensões|
|Operações de Gravação do SO Por Disco/s|Operações de Gravação do Disco do SO/s (Preteridos)|CountPerSecond|Average|Gravar IOPS de um único disco durante o período de monitoramento para disco do SO|Sem dimensões|
|QD do SO Por Disco|QD de Disco do SO (Preterido)|Contagem|Average|Profundidade da Fila do Disco do SO (ou Comprimento da Fila)|Sem dimensões|
|Bytes de Leitura do Disco de Dados/s|Bytes de Leitura do Disco de Dados/s (Versão Prévia)|CountPerSecond|Average|Leitura de Bytes/s de um único disco durante o período de monitoramento|LUN, VMName|
|Bytes de Gravação do Disco de Dados/s|Bytes de Gravação do Disco de Dados/s (Versão Prévia)|CountPerSecond|Average|Bytes/s gravados em um único disco durante o período de monitoramento|LUN, VMName|
|Operações de Leitura do Disco de Dados/s|Operações de Leitura do Disco de Dados/s (Versão Prévia)|CountPerSecond|Average|Ler IOPS de um único disco durante o período de monitoramento|LUN, VMName|
|Operações de Gravação do Disco de Dados/s|Operações de Gravação do Disco de Dados/s (Versão Prévia)|CountPerSecond|Average|Gravar IOPS de um único disco durante o período de monitoramento|LUN, VMName|
|Profundidade da Fila de Disco de Dados|Profundidade da Fila de Disco de Dados (Versão prévia)|Count|Average|Profundidade da Fila do Disco de Dados (ou Tamanho da Fila)|LUN, VMName|
|Bytes de Leitura do Disco do SO/s|Bytes de Leitura do Disco do SO/s (Versão Prévia)|CountPerSecond|Average|Leitura de Bytes/s de um único disco durante o período de monitoramento do disco do SO|VMName|
|Bytes de Gravação do Disco do SO/s|Bytes de Gravação do Disco do SO/s (Versão Prévia)|CountPerSecond|Average|Bytes/s gravados em um único disco durante o período de monitoramento do disco do SO|VMName|
|Operações de Leitura do Disco do SO/s|Operações de Leitura do Disco do SO/s (Versão Prévia)|CountPerSecond|Average|Ler IOPS de um único disco durante o período de monitoramento para disco do SO|VMName|
|Operações de Gravação do Disco do SO/s|Operações de Gravação do Disco do SO/s (Versão Prévia)|CountPerSecond|Average|Gravar IOPS de um único disco durante o período de monitoramento para disco do SO|VMName|
|Profundidade da Fila de Disco do SO|Profundidade da Fila de Disco do SO (Versão prévia)|Count|Average|Profundidade da Fila do Disco do SO (ou Comprimento da Fila)|VMName|
|Fluxos de Entrada|Fluxos de Entrada (Versão prévia)|Count|Average|Fluxos de Entrada serão o número de fluxos atuais na direção de entrada (tráfego para dentro da VM)|VMName|
|Fluxos de Saída|Fluxos de Saída (Versão prévia)|Contagem|Average|Fluxos de Saída serão o número de fluxos atuais na direção de saída (tráfego para fora da VM)|VMName|
|Taxa Máxima de Criação de Fluxos de Entrada|Taxa Máxima de Criação de Fluxos de Entrada (Versão Prévia)|CountPerSecond|Average|A taxa máxima de criação de fluxos de entrada (o tráfego que entra na VM)|VMName|
|Taxa Máxima de Criação de Fluxos de Saída|Taxa Máxima de Criação de Fluxos de Saída (Versão Prévia)|CountPerSecond|Average|A taxa máxima de criação de fluxos de saída (o tráfego que sai da VM)|VMName|
|Acerto de Leitura do Cache de Disco de Dados Premium|Acerto de Leitura do Cache de Disco de Dados Premium (Versão prévia)|Percent|Average|Acerto de Leitura do Cache de Disco de Dados Premium|LUN, VMName|
|Erro de Leitura do Cache de Disco de Dados Premium|Erro de Leitura do Cache de Disco de Dados Premium (Versão prévia)|Percent|Average|Erro de Leitura do Cache de Disco de Dados Premium|LUN, VMName|
|Acerto de Leitura do Cache de Disco do SO Premium|Acerto de Leitura do Cache de Disco do SO Premium (Versão Prévia)|Percent|Average|Acerto de Leitura do Cache de Disco do SO Premium|VMName|
|Erro de Leitura do Cache de Disco de SO Premium|Erro de Leitura do Cache de Disco de SO Premium (Versão prévia)|Percent|Average|Erro de Leitura do Cache de Disco de SO Premium|VMName|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|CpuUsage|Uso da CPU|Count|Average|Uso da CPU em todos os núcleos em milinúcleos.|containerName|
|MemoryUsage|Uso de Memória|Bytes|Average|Uso total de memória em bytes.|containerName|
...
|SuccessfulPullCount|Contagem de pull bem-sucedida|Contagem|Average|Número de pulls de imagem bem-sucedidos|Sem dimensões|
|TotalPushCount|Contagem total de push|Contagem|Average|Número de envios por push de imagem no total|Sem dimensões|
|SuccessfulPushCount|Contagem de push bem-sucedida|Contagem|Average|Número de Pushes de imagem bem-sucedidos|Sem dimensões|
...
|DCIKpisCount|Contagem de KPIs|Contagem|Last||Sem dimensões|
...
|DCIPredictionsCount|Contagem de Previsão|Count|Last||Sem dimensões|

## <a name="microsoftdataboxedgedataboxedgedevices"></a>Microsoft.DataBoxEdge/dataBoxEdgeDevices

|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|NICReadThroughput|Produtividade de Leitura (Rede)|BytesPerSecond|Average|A produtividade de leitura do adaptador de rede no dispositivo no período de relatório para todos os volumes no gateway.|InstanceName|
|NICWriteThroughput|Produtividade de Gravação (Rede)|BytesPerSecond|Average|A produtividade de gravação do adaptador de rede no dispositivo no período de relatório para todos os volumes no gateway.|InstanceName|
|CloudReadThroughputPerShare|Produtividade de Download de Nuvem (Compartilhamento)|BytesPerSecond|Average|A produtividade do download para o Azure de um compartilhamento durante o período de relatório.|Compartilhar|
|CloudUploadThroughputPerShare|Produtividade de Upload para Nuvem (Compartilhamento)|BytesPerSecond|Average|A produtividade de upload para o Azure de um compartilhamento durante o período de relatório.|Compartilhar|
|BytesUploadedToCloudPerShare|Bytes de Nuvem Carregados (Compartilhamento)|Bytes|Average|O número total de bytes que é carregado para o Azure de um compartilhamento durante o período de relatório.|Compartilhar|
...
|CloudUploadThroughput|Produtividade de Upload para Nuvem|BytesPerSecond|Average|A taxa de transferência de uploads na nuvem para o Azure durante o período do relatório.|Sem dimensões|
|CloudReadThroughput|Produtividade de Download de Nuvem|BytesPerSecond|Average|A produtividade de download da nuvem para o Azure durante o período do relatório.|Sem dimensões|
|BytesUploadedToCloud|Bytes de Nuvem Carregados (Dispositivo)|Bytes|Average|O número total de bytes que é carregado para o Azure de um dispositivo durante o período de relatório.|Sem dimensões|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|FailedRuns|Execuções com Falhas|Contagem|Total||pipelinename, ActivityName|
...
|cpu_percent|Porcentagem de CPU|Percent|Average|Porcentagem de CPU|Sem dimensões|
|memory_percent|Porcentagem de memória|Percent|Average|Porcentagem de memória|Sem dimensões|
|IOPS|IOPS|Contagem|Average|Operações de e/s por segundo|Sem dimensões|
|storage_percent|Porcentagem de armazenamento|Percent|Average|Porcentagem de armazenamento|Sem dimensões|
|storage_used|Armazenamento usado|Bytes|Average|Armazenamento usado|Sem dimensões|
|active_connections|Conexões ativas|Count|Average|Conexões ativas|Sem dimensões|
|network_bytes_egress|Saída de Rede|Bytes|Total|Saída de Rede em conexões ativas|Sem dimensões|
...
|CassandraRequests|Solicitações do Cassandra|Count|Count|Número de solicitações Cassandra feitas|DatabaseName, CollectionName, região, OperationType, ResourceType, ErrorCode|
...
|TotalRequests|Total de Solicitações|Count|Contagem|Número de solicitações feitas|DatabaseName, CollectionName, região, StatusCode, OperationType|
...
|UserErrors|Erros do Usuário.|Contagem|Total|Erros do Usuário para Microsoft.EventHub.|EntityName, |
...
|IncomingRequests|Solicitações de Entrada|Contagem|Total|Solicitações de Entrada para Microsoft.EventHub.|EntityName|
...
|ActiveConnections|ActiveConnections|Count|Average|Conexões Totalmente Ativas para Microsoft.EventHub.|Sem dimensões|
...
|CaptureBacklog|Capture a Lista de Pendências.|Contagem|Total|Capture a Lista de Pendências para o Microsoft.EventHub.|EntityName|
...
|EHABL|Arquivar mensagens de pendências (preterido)|Count|Total|Mensagens de arquivamento do hub de eventos no backlog para um namespace (preterido)|Sem dimensões|
...
|ThrottledRequests|Solicitações Limitadas. (Versão Prévia)|Contagem|Total|Solicitações Limitadas para Microsoft.EventHub. (Versão Prévia)|Sem dimensões|
...
|ActiveConnections|ActiveConnections (Versão Prévia)|Count|Average|Conexões Totalmente Ativas para Microsoft.EventHub. (Versão Prévia)|Sem dimensões|
|ConnectionsOpened|Conexões Abertas. (Versão Prévia)|Count|Average|Conexões Abertas para o Microsoft.EventHub. (Versão Prévia)|Sem dimensões|
|ConnectionsClosed|Conexões Fechadas. (Versão Prévia)|Count|Average|Conexões Fechadas para o Microsoft.EventHub. (Versão Prévia)|Sem dimensões|
...
|CapturedMessages|Mensagens Capturadas. (Versão Prévia)|Contagem|Total|Mensagens Capturadas para o Microsoft.EventHub. (Versão Prévia)|Sem dimensões|
...
|CPU|CPU (versão prévia)|Percent|Máximo|Utilização de CPU para o Cluster do Hub de Eventos como um porcentual|Role|
|AvailableMemory|Memória disponível (versão prévia)|Contagem|Máximo|Memória disponível para o Cluster do Hub de Eventos em bytes|Role|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|ObservedMetricValue|Valor de Métrica Observado|Contagem|Average|O valor computado pelo dimensionamento automático quando executado|MetricTriggerSource|
|MetricThreshold|Limite de Métrica|Count|Average|O limite de dimensionamento automático configurado quando a autoescala foi executada.|MetricTriggerRule|
|ObservedCapacity|Capacidade Observada|Count|Average|A capacidade relatada ao dimensionamento automático quando executado.|Sem dimensões|
|ScaleActionsInitiated|Ações de Escala Iniciadas|Contagem|Total|A direção da operação da escala.|ScaleDirection|
...
|browserTimings/sendDuration|Enviar tempo de solicitação|MilliSeconds|Average|Tempo entre a conexão de rede e receber o primeiro byte.|Sem dimensões|
|browserTimings/totalDuration|Procurar o tempo de carregamento de página|MilliSeconds|Average|Tempo desde a solicitação do usuário até a DOM, stylesheets, scripts e imagens são carregados.|Sem dimensões|
|dependencies/count|Chamadas de dependência|Contagem|Count|Contagem de chamadas feitas pelo aplicativo a recursos externos.|dependency/type, dependency/performanceBucket, dependency/success, operation/synthetic, cloud/roleInstance, cloud/roleName|
...
|performanceCounters/requestsInQueue|Solicitações HTTP na fila do aplicativo|Contagem|Average|Comprimento da fila de solicitações do aplicativo.|cloud/roleInstance|
|performanceCounters/requestsPerSecond|Taxa de solicitação HTTP|CountPerSecond|Average|Taxa de todas as solicitações para o aplicativo por segundo de ASP.NET.|cloud/roleInstance|
|performanceCounters/exceptionsPerSecond|Taxa de exceção|CountPerSecond|Average|Contagem de exceções manipuladas e não manipuladas relatadas para o Windows, incluindo exceções .NET e exceções não gerenciadas que são convertidas para exceções .NET.|cloud/roleInstance|
|performanceCounters/processIOBytesPerSecond|Taxa de E/S do processo|BytesPerSecond|Average|Total de bytes por segundo de leitura e gravação de arquivos, redes e dispositivos.|cloud/roleInstance|
...
|performanceCounters/processPrivateBytes|Bytes privados do processo|Bytes|Average|Memória atribuída exclusivamente a processos monitorados do aplicativo.|cloud/roleInstance|
|requests/duration|Tempo de resposta do servidor|MilliSeconds|Average|Tempo entre o recebimento de uma solicitação HTTP e a conclusão do envio da resposta.|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, request/success, cloud/roleName|
...
|requests/failed|Solicitações com falha|Count|Contagem|Contagem de solicitações HTTP marcadas como com falha. Na maioria dos casos, essas são solicitações com um código de resposta >= 400 e diferente de 401.|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, cloud/roleName|
...
|exceptions/count|Exceções|Contagem|Count|Contagem combinada de todas as exceções não detectadas.|cloud/roleName, cloud/roleInstance, client/type|
|exceptions/browser|Exceções do navegador|Contagem|Count|Contagem de exceções não capturadas lançadas no navegador.|Sem dimensões|
|exceptions/server|Exceções do servidor|Contagem|Count|Contagem de exceções não capturadas lançadas no aplicativo do servidor.|cloud/roleName, cloud/roleInstance|
|traces/count|Rastreamentos|Contagem|Count|Contagem do documento de rastreamento|trace/severityLevel, operation/synthetic, cloud/roleName, cloud/roleInstance|

## <a name="microsoftkeyvaultvaults"></a>Microsoft.KeyVault/vaults

|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|ServiceApiHit|Total de resultados da API de serviço|Contagem|Count|Número do total de resultados da API de serviço|ActivityType, ActivityName|
|ServiceApiLatency|Latência da API de Serviço Geral|Milissegundos|Average|Latência geral das solicitações de API de serviço|ActivityType, ActivityName, StatusCode|
|ServiceApiResult|Resultados da API de Serviço Total|Count|Contagem|Número do total de resultados da API de serviço|ActivityType, ActivityName, StatusCode|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|Uso|Uso|Contagem|Count|Contagem de chamadas à API|ApiCategory, ApiName|
...
|RunsFailed|Execuções com falha|Contagem|Total|Número de falhas de execuções de fluxo de trabalho.|Sem dimensões|
|RunsCancelled|Execuções canceladas|Contagem|Total|Número de execuções de fluxo de trabalho canceladas.|Sem dimensões|
|RunLatency|Latência da execução|Segundos|Average|Latência de execuções de fluxo de trabalho concluídas.|Sem dimensões|
...
|ActionSuccessLatency|Latência das ações bem sucedidas |Segundos|Average|Latência de ações de fluxo de trabalho bem-sucedidas.|Sem dimensões|
...
|TriggerLatency|Latência do gatilho |Segundos|Average|Latência de gatilhos de fluxo de trabalho concluídos.|Sem dimensões|
|TriggerFireLatency|Latência do gatilho acionado |Segundos|Average|Latência de gatilhos de fluxo de trabalho disparados.|Sem dimensões|
|TriggerSuccessLatency|Latência do gatilho bem sucedido |Segundos|Average|Latência de gatilhos de fluxo de trabalho bem-sucedidos.|Sem dimensões|
|TriggerThrottledEvents|Eventos de restrição do gatilho|Contagem|Total|Número de eventos de fluxo de trabalho acelerados por gatilho.|Sem dimensões|
...
|BillableTriggerExecutions|Execuções de gatilho faturáveis|Contagem|Total|Número de execuções de gatilho de fluxo de trabalho sendo faturadas.|Sem dimensões|
...
|BillingUsageStorageConsumption|Uso de Cobrança para Execuções de Consumo de Armazenamento|Contagem|Total|Número de execuções de consumo de armazenamento sendo faturadas.|Sem dimensões|
|BillingUsageNativeOperation|Uso de Cobrança para Execuções de Operação Nativa|Count|Total|Número de execuções de operação nativa sendo faturadas.|Sem dimensões|
|BillingUsageStandardConnector|Uso de Cobrança para Execuções de Conector Padrão|Contagem|Total|Número de execuções do conector padrão sendo faturadas.|Sem dimensões|
...
|RunsSucceeded|Execuções bem sucedidas|Count|Total|Número de execuções de fluxo de trabalho bem-sucedidas.|Sem dimensões|
|RunsFailed|Execuções com falha|Contagem|Total|Número de falhas de execuções de fluxo de trabalho.|Sem dimensões|
|RunsCancelled|Execuções canceladas|Count|Total|Número de execuções de fluxo de trabalho canceladas.|Sem dimensões|
|RunLatency|Latência da execução|Segundos|Average|Latência de execuções de fluxo de trabalho concluídas.|Sem dimensões|
|RunSuccessLatency|Latência de execução bem sucedida|Segundos|Average|Latência de execuções de fluxo de trabalho bem-sucedidas.|Sem dimensões|
|RunThrottledEvents|Eventos de restrição de execução|Count|Total|Número de ações de fluxo de trabalho ou de disparos de eventos acelerados.|Sem dimensões|
...
|ActionLatency|Latência da ação |Segundos|Average|Latência das ações de fluxo de trabalho concluídas.|Sem dimensões|
|ActionSuccessLatency|Latência das ações bem sucedidas |Segundos|Average|Latência de ações de fluxo de trabalho bem-sucedidas.|Sem dimensões|
|ActionThrottledEvents|Eventos de restrição de ações|Contagem|Total|Número de eventos acelerados da ação de fluxo de trabalho.|Sem dimensões|
|TriggersStarted|Gatilhos iniciados |Count|Total|Número de gatilhos de fluxo de trabalho iniciados.|Sem dimensões|
|TriggersCompleted|Gatilhos concluídos |Count|Total|Número de gatilhos de fluxo de trabalho concluídos.|Sem dimensões|
|TriggersSucceeded|Gatilhos bem sucedidos |Count|Total|Número de gatilhos de fluxo de trabalho bem-sucedidos.|Sem dimensões|
|TriggersFailed|Gatilhos com falha |Count|Total|Número de falhas nos gatilhos de fluxo de trabalho.|Sem dimensões|
|TriggersSkipped|Gatilhos ignorados|Count|Total|Número de gatilhos de fluxo de trabalho ignorados.|Sem dimensões|
|TriggersFired|Gatilhos acionados |Contagem|Total|Número de gatilhos de fluxo de trabalho disparados.|Sem dimensões|
|TriggerLatency|Latência do gatilho |Segundos|Average|Latência de gatilhos de fluxo de trabalho concluídos.|Sem dimensões|
|TriggerFireLatency|Latência do gatilho acionado |Segundos|Average|Latência de gatilhos de fluxo de trabalho disparados.|Sem dimensões|
|TriggerSuccessLatency|Latência do gatilho bem sucedido |Segundos|Average|Latência de gatilhos de fluxo de trabalho bem-sucedidos.|Sem dimensões|
|TriggerThrottledEvents|Eventos de restrição do gatilho|Contagem|Total|Número de eventos de fluxo de trabalho acelerados por gatilho.|Sem dimensões|
|IntegrationServiceEnvironmentWorkflowProcessorUsage|Uso do Processador de Fluxo de Trabalho para o Ambiente de Serviço de Integração|Percent|Average|Uso do processador de fluxo de trabalho para o ambiente de serviço de integração.|Sem dimensões|
|IntegrationServiceEnvironmentWorkflowMemoryUsage|Uso de Memória do Fluxo de Trabalho para o Ambiente de Serviço de Integração|Percent|Average|Uso de memória do fluxo de trabalho para o ambiente de serviço de integração.|Sem dimensões|
|IntegrationServiceEnvironmentConnectorProcessorUsage|Uso do Processador de Conector para o Ambiente de Serviço de Integração|Percent|Average|Uso do processador de conector para o ambiente de serviço de integração.|Sem dimensões|
|IntegrationServiceEnvironmentConnectorMemoryUsage|Uso de Memória do Conector para o Ambiente de Serviço de Integração|Percent|Average|Uso de memória do conector para o ambiente de serviço de integração.|Sem dimensões|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|Uso|Uso|Contagem|Count|Contagem de chamadas à API|ApiCategory, ApiName, ResultType, ResponseCode|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|VipAvailability|Disponibilidade do Caminho de Dados|Contagem|Average|Disponibilidade média do caminho de dados do Balanceador de Carga por duração de tempo|FrontendIPAddress, FrontendPort|
...
|CapacityUnits|Unidades de capacidade atuais|Count|Average|Unidades de capacidade consumidas|Sem dimensões|
...
|registration.create|Operação de criação de registro|Contagem|Total|A contagem de criações de registro bem-sucedidas.|Sem dimensões|
...
|registration.get|Operações de leitura do registro|Contagem|Total|A contagem de consultas de registro bem-sucedidas.|Sem dimensões|
|registration.delete|Operação de exclusão de registro|Contagem|Total|A contagem de exclusões de registro bem-sucedidas.|Sem dimensões|
...
|outgoing.allpns.invalidpayload|Erros de conteúdo|Contagem|Total|A contagem de pushes que falharam porque o PNS retornou um erro de conteúdo defeituoso.|Sem dimensões|
...
|outgoing.allpns.channelerror|Erros de canal|Contagem|Total|A contagem de pushes que falharam porque o canal era inválido, não estava associado ao aplicativo correto, foi limitado ou expirado.|Sem dimensões|
...
|outgoing.apns.invalidcredentials|Erros de autorização do APNS|Contagem|Total|A contagem de pushes que falharam porque o PNS não aceitou as credenciais fornecidas ou porque as credenciais estão bloqueadas.|Sem dimensões|
...
|notificationhub.pushes|Todas as notificações de saída|Contagem|Total|Todas as notificações de saída no hub de notificação|Sem dimensões|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|Average_% Free Inodes|% de Inodes livres|Contagem|Average|Average_% Free Inodes|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Disk Write Bytes/sec|Bytes/s Gravados em Disco|Contagem|Average|Average_Disk Write Bytes/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_% Used Swap Space|% de Espaço de Permuta Usado|Count|Average|Average_% Used Swap Space|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Available MBytes Swap|MBytes de Espaço de Permuta Disponíveis|Contagem|Average|Average_Available MBytes Swap|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Total Rx Errors|Total de Erros de Rx|Count|Average|Average_Total Rx Errors|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Avg. de segundos/Gravação do Disco|Média de segundos/Gravação do Disco|Contagem|Average|Average_Avg. de segundos/Gravação do Disco|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
|Average_Physical Disk Bytes/sec|Bytes/s do Disco Físico|Count|Average|Average_Physical Disk Bytes/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Size Stored In Paging Files|Tamanho Armazenado em Arquivos de Paginação|Contagem|Average|Average_Size Stored In Paging Files|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Current Disk Queue Length|Comprimento da Fila do Disco Atual|Contagem|Average|Average_Current Disk Queue Length|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
|Average_Disk Reads/sec|Leituras de Disco/s|Count|Average|Average_Disk Reads/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
|Average_Disk Transfers/sec|Transferências de Disco/s|Count|Average|Average_Disk Transfers/sec|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_% Committed Bytes In Use|% de Bytes Confirmados em Uso|Contagem|Average|Average_% Committed Bytes In Use|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Average_Processor Queue Length|Tamanho da fila do processador|Contagem|Average|Average_Processor Queue Length|Computer, ObjectName, InstanceName, CounterPath, SourceSystem|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|ListenerConnections-Success|ListenerConnections-Success|Contagem|Total|ListenerConnections bem-sucedido para Microsoft.Relay.|EntityName|
|ListenerConnections-ClientError|ListenerConnections-ClientError|Contagem|Total|ClientError no ListenerConnections para Microsoft.Relay.|EntityName|
|ListenerConnections-ServerError|ListenerConnections-ServerError|Contagem|Total|ServerError no ListenerConnections para Microsoft.Relay.|EntityName|
...
|SenderConnections-ClientError|SenderConnections-ClientError|Contagem|Total|ClientError no SenderConnections para Microsoft.Relay.|EntityName|
...
|IncomingRequests|Solicitações de Entrada (Versão Prévia)|Contagem|Total|Solicitações de Entrada para Microsoft.ServiceBus. (Versão Prévia)|EntityName|
|IncomingMessages|Mensagens de Entrada (Versão Prévia)|Contagem|Total|Mensagens de Entrada para Microsoft.ServiceBus. (Versão Prévia)|EntityName|
...
|ActiveConnections|ActiveConnections (Versão Prévia)|Contagem|Total|Conexões Totalmente Ativas para Microsoft.ServiceBus. (Versão Prévia)|Sem dimensões|
|Tamanho|Tamanho (versão prévia)|Bytes|Average|Tamanho de um Fila/Tópico em Bytes. (Versão Prévia)|EntityName|
|Mensagens|Contagem de mensagens em uma Fila/Tópico. (Versão Prévia)|Contagem|Average|Contagem de mensagens em uma Fila/Tópico. (Versão Prévia)|EntityName|
|ActiveMessages|Contagem de mensagens ativas em uma Fila/Tópico. (Versão Prévia)|Count|Average|Contagem de mensagens ativas em uma Fila/Tópico. (Versão Prévia)|EntityName|
|DeadletteredMessages|Contagem de mensagens mortas em uma Fila/Tópico. (Versão Prévia)|Contagem|Average|Contagem de mensagens mortas em uma Fila/Tópico. (Versão Prévia)|EntityName|
|ScheduledMessages|Contagem de mensagens agendadas em uma Fila/Tópico. (Versão Prévia)|Contagem|Average|Contagem de mensagens agendadas em uma Fila/Tópico. (Versão Prévia)|EntityName|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|AllocatedCpu|AllocatedCpu|Contagem|Average|CPU alocada para este contêiner em milicores|ApplicationName, ServiceName, CodePackageName, ServiceId|
...
|ActualCpu|ActualCpu|Contagem|Average|Uso real da CPU em milicores|ApplicationName, ServiceName, CodePackageName, ServiceId|
...
|dtu_consumption_percent|Porcentual de DTU|Percent|Average|Porcentual de DTU|Sem dimensões|
...
|storage_percent|Porcentagem de espaço de dados usada|Percent|Máximo|Percentual de tamanho do banco de dados|Sem dimensões|
...
|dtu_used|DTU usado|Contagem|Average|DTU usado|Sem dimensões|
|cpu_limit|Limite de CPU|Count|Average|Limite de CPU|Sem dimensões|
|cpu_used|CPU usada|Contagem|Average|CPU usada|Sem dimensões|
...
|app_cpu_billed|CPU do aplicativo cobrada|Count|Total|CPU do aplicativo cobrada|Sem dimensões|
|app_cpu_percent|Porcentagem de CPU do aplicativo|Percent|Average|Porcentagem de CPU do aplicativo|Sem dimensões|
|app_memory_percent|Porcentagem de memória usada do aplicativo|Percent|Average|Porcentagem de memória usada do aplicativo|Sem dimensões|
...
|workers_percent|Porcentagem de funcionários|Percent|Average|Porcentagem de funcionários|Sem dimensões|
|sessions_percent|Porcentagem de sessões|Percent|Average|Porcentagem de sessões|Sem dimensões|
|eDTU_limit|Limite de eDTU|Contagem|Average|Limite de eDTU|Sem dimensões|
...
|cpu_used|CPU usada|Count|Average|CPU usada|Sem dimensões|
|allocated_data_storage|Espaço de dados alocado|Bytes|Average|Espaço de dados alocado|Sem dimensões|
...
|reserved_storage_mb|Espaço de armazenamento reservado|Count|Average|Espaço de armazenamento reservado|Sem dimensões|
|storage_space_used_mb|Espaço de armazenamento usado|Count|Average|Espaço de armazenamento usado|Sem dimensões|
|io_requests|Contagem de solicitações de E/S|Contagem|Average|Contagem de solicitações de E/S|Sem dimensões|
...
|IndexCapacity|Capacidade de Índice|Bytes|Average|A quantidade de armazenamento usado pelo Índice ADLS Gen2 (Hierárquico) em bytes.|Sem dimensões|
|Transações|Transações|Contagem|Total|O número de solicitações feitas a um serviço de armazenamento ou à operação de API especificada. Esse número inclui solicitações bem-sucedidas e com falha, bem como solicitações que produziram erros. Use a dimensão ResponseType para obter o número de tipo diferente de resposta.|ResponseType, GeoType, ApiName, Authentication|
|Entrada|Entrada|Bytes|Total|A quantidade de dados de entrada, em bytes. Esse número inclui a entrada de um cliente externo no Armazenamento do Azure, bem como a entrada no Azure.|GeoType, ApiName, Authentication|
|Saída|Saída|Bytes|Total|A quantidade de dados de saída, em bytes. Esse número inclui a saída de um cliente externo no Armazenamento do Azure, bem como a saída no Azure. Como resultado, esse número não reflete a saída faturável.|GeoType, ApiName, Authentication|
|SuccessServerLatency|Latência de Servidor Com Sucesso|Milissegundos|Média|A latência média usada pelo Armazenamento do Azure para processar uma solicitação bem-sucedida, em milissegundos. Esse valor não inclui a latência de rede especificada em AverageE2ELatency.|GeoType, ApiName, Authentication|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|FileCapacity|Capacidade do Arquivo|Bytes|Average|A quantidade de armazenamento usada pelo serviço de Arquivo da conta de armazenamento, em bytes.|Sem dimensões|
|FileCount|Contagem de Arquivos|Count|Average|O número de arquivos no serviço de Arquivo da conta de armazenamento.|Sem dimensões|
|FileShareCount|Contagem de Compartilhamentos de Arquivo|Count|Average|O número de compartilhamentos de arquivo no serviço de Arquivo da conta de armazenamento.|Sem dimensões|
|Transações|Transações|Contagem|Total|O número de solicitações feitas a um serviço de armazenamento ou à operação de API especificada. Esse número inclui solicitações bem-sucedidas e com falha, bem como solicitações que produziram erros. Use a dimensão ResponseType para obter o número de tipo diferente de resposta.|ResponseType, GeoType, ApiName, Authentication|
|Entrada|Entrada|Bytes|Total|A quantidade de dados de entrada, em bytes. Esse número inclui a entrada de um cliente externo no Armazenamento do Azure, bem como a entrada no Azure.|GeoType, ApiName, Authentication|
|Saída|Saída|Bytes|Total|A quantidade de dados de saída, em bytes. Esse número inclui a saída de um cliente externo no Armazenamento do Azure, bem como a saída no Azure. Como resultado, esse número não reflete a saída faturável.|GeoType, ApiName, Authentication|
|SuccessServerLatency|Latência de Servidor Com Sucesso|Milissegundos|Média|A latência média usada pelo Armazenamento do Azure para processar uma solicitação bem-sucedida, em milissegundos. Esse valor não inclui a latência de rede especificada em AverageE2ELatency.|GeoType, ApiName, Authentication|
...
|Métrica|Nome de exibição da métrica|Unidade|Tipo de agregação|DESCRIÇÃO|Dimensões|
|---|---|---|---|---|---|
|DiskReadBytesPerSecond|Bytes de Leitura no Disco/s|BytesPerSecond|Average|Taxa de transferência média do disco devido a operações de leitura no período de exemplo.|Sem dimensões|
|DiskWriteBytesPerSecond|Bytes de Gravação no Disco/s|BytesPerSecond|Average|Taxa de transferência média do disco devido a operações de gravação no período de exemplo.|Sem dimensões|
...
|Operações de Leitura de Disco por segundo|Operações de leitura de disco/Seg|CountPerSecond|Average|O número médio de operações de leitura de e/s no período de exemplo anterior. Observe que essas operações podem ser de tamanho variável.|Sem dimensões|
|Operações/s de Gravação do Disco|Operações de gravação de disco/Seg|CountPerSecond|Average|O número médio de operações de gravação de e/s no período de exemplo anterior. Observe que essas operações podem ser de tamanho variável.|Sem dimensões|
...
|CurrentAssemblies|Módulos (Assemblies) Atuais|Count|Average|Módulos (Assemblies) Atuais|Instância|
...
|TotalAppDomains|Total de domínios de aplicativo|Contagem|Average|Total de domínios de aplicativo|Instância|
|TotalAppDomainsUnloaded|Total de domínios de aplicativo descarregados|Contagem|Average|Total de domínios de aplicativo descarregados|Instância|
...
|MediumAppServicePlanInstances|Funções de trabalho médias do Plano do Serviço de Aplicativo|Contagem|Média|Trabalhadores do Plano do Serviço de Aplicativo Médio|Sem dimensões|
...
|WorkersAvailable|Trabalhadores Disponíveis|Count|Average|Trabalhadores Disponíveis|Sem dimensões|
