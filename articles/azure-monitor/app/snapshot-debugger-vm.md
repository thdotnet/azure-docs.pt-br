...
Se o ASP.NET ou ASP.NET core execuções de aplicativo no serviço de aplicativo do Azure, ele é altamente recomendável [habilitar o depurador de instantâneo por meio da página de portal do Application Insights](snapshot-debugger-appservice.md?toc=/azure/azure-monitor/toc.json). No entanto, se seu aplicativo requer uma configuração personalizada do depurador de instantâneo ou uma versão prévia do .NET core, em seguida, essa instrução deve ser seguida ***Além disso*** as instruções de [habilitar por meio do a página de portal do Application Insights](snapshot-debugger-appservice.md?toc=/azure/azure-monitor/toc.json).

Se seu aplicativo é executado no Azure Service Fabric, serviço de nuvem, máquinas virtuais ou computadores locais, as instruções a seguir devem ser usadas. 
...
## <a name="configure-snapshot-collection-for-applications-using-aspnet-core-20-or-above"></a>Configurar a coleta de instantâneo para aplicativos que usam o ASP.NET Core 2.0 ou superior
...
3. Modifique a classe `Startup` do seu aplicativo para adicionar e configurar o processador de telemetria do Coletor de Instantâneo.
    1. Se [snapshotcollector](https://www.nuget.org/packages/Microsoft.ApplicationInsights.SnapshotCollector) NuGet versão 1.3.5 do pacote ou acima é usado, em seguida, adicione o seguinte usando as instruções para `Startup.cs`.

       ```csharp
            using Microsoft.ApplicationInsights.SnapshotCollector;
       ```

       Adicione o seguinte ao final do método ConfigureServices na `Startup` classe `Startup.cs`.
       ...
       ```
    2. Se [snapshotcollector](https://www.nuget.org/packages/Microsoft.ApplicationInsights.SnapshotCollector) NuGet 1.3.4 de versão do pacote ou abaixo é usado, em seguida, adicione as seguintes instruções using ao `Startup.cs`.
