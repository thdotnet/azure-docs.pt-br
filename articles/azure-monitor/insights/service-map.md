...
Este artigo descreve os detalhes de integração e como usar o Mapa do Serviço. Para obter mais informações sobre como configurar o Mapa do Serviço e os agentes de integração, consulte [Configurar a solução Mapa do Serviço no Azure]( service-map-configure.md).

>[!NOTE]
>Se o Mapa do Serviço já foi implantado, também será possível exibir os mapas no Azure Monitor para VMs, que inclui recursos adicionais para monitorar a integridade e o desempenho da VM. Para saber mais, consulte [visão geral do Azure Monitor para VMs](../../azure-monitor/insights/vminsights-overview.md).
...
Consulte a [seção Solução de problemas do documento Configurando o mapa do serviço]( service-map-configure.md#troubleshooting).
