...
* **Um espaço de trabalho do Log Analytics.**
    ...
    O Azure Monitor para contêineres oferece suporte a um espaço de trabalho do Log Analytics nas regiões listadas no Azure [produtos por região](https://azure.microsoft.com/global-infrastructure/services/?regions=all&products=monitor), exceto a região **gov. EUA-Virgínia**.

    Você pode criar um espaço de trabalho quando você habilita o monitoramento de seu novo cluster do AKS ou permitir que a experiência de integração criar um espaço de trabalho padrão no grupo de recursos padrão da assinatura de cluster do AKS. Se optar por criá-lo por conta própria, será possível criá-lo por meio do [Azure Resource Manager](../platform/template-workspace-configuration.md), do [PowerShell](../scripts/powershell-sample-create-workspace.md?toc=%2fpowershell%2fmodule%2ftoc.json) ou no [portal do Azure](../learn/quick-create-workspace.md). Para obter uma lista de pares de mapeamento com suporte usados para o espaço de trabalho padrão, consulte [mapeamento de região para o Azure Monitor para contêineres](container-insights-region-mapping.md).
