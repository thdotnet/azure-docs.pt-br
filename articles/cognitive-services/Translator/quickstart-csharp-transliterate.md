...
Neste início rápido, você aprenderá a transcrever texto (converter) de um script para outro usando .NET Core (C#), C# 7.1 ou posterior e a API REST de Tradução de Texto. No exemplo fornecido, o japonês é transliterado para usar o alfabeto latino.
...
Na classe `Program`, crie uma função assíncrona chamada `TransliterateTextRequest()`. Esta função usará quatro argumentos: `subscriptionKey`, `host`, `route` e `inputText`.
...
É possível observar que em `Main`, você declara `subscriptionKey`, `host`, `route` e o script para transcrever `textToTransliterate`.
