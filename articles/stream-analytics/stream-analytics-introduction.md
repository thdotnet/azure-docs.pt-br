...
ms.locfileid: "65789596"
---
# <a name="what-is-azure-stream-analytics"></a>O que é o Azure Stream Analytics?

O Azure Stream Analytics é um mecanismo de processamento de eventos desenvolvido para examinar grandes volumes de dados de streaming. É possível identificar padrões e relações em informações extraídas de diversas fontes de entrada, incluindo dispositivos, sensores, sites, feeds de mídias sociais e aplicativos. Esses padrões podem ser usados para disparar outras ações em downstream, como alertas ou informações de feed para uma ferramenta de relatórios, ou para armazenar dados transformados para uso posterior.

Os cenários a seguir são exemplos de quando você pode usar o Azure Stream Analytics:

* Fusão do sensor de IoT (Internet das Coisas) e da análise em tempo real na telemetria do dispositivo
...
Um trabalho do Azure Stream Analytics consiste em uma entrada, uma consulta de transformação e uma saída. Quando eventos de seu software ou dispositivos são ingeridos por Hubs de Eventos do Azure, pelo Hub IoT do Azure ou pelo Armazenamento de Blobs do Azure, você pode especificar um ou mais desses serviços como a fonte de entrada de seu trabalho. A consulta de transformação, que tem como base a linguagem de consulta SQL, é usada para filtrar, classificar, agregar e associar dados de streaming com facilidade por um período de tempo. Você pode ajustar as opções de ordenação de eventos e a duração das janelas de tempo na execução de operações de agregação.
...
* Envie dados a um painel do Power BI para visualização em tempo real.
...
É fácil começar a usar o Azure Stream Analytics. Bastam alguns cliques para se conectar a várias fontes e coletores, assim criando um pipeline de ponta a ponta. O Stream Analytics pode se conectar a [Hubs de Eventos do Azure](/azure/event-hubs/) e a [Hub IoT do Azure](/azure/iot-hub/) para a ingestão de dados de fluxo, e ao [Armazenamento de Blobs do Azure](/azure/storage/storage-introduction) para ingerir dados históricos. A entrada de trabalho também pode incluir dados de referência estáticos ou com alteração lenta do Armazenamento de Blobs do Azure ou do [Banco de Dados SQL](stream-analytics-use-reference-data.md#azure-sql-database-preview), que você pode unir a dados de streaming para executar operações de pesquisa.

O Stream Analytics pode rotear a saída de trabalho para vários sistemas de armazenamento, como o [Armazenamento de Blob do Azure](/azure/storage/storage-introduction), o [Banco de Dados SQL do Azure](/azure/sql-database/), o [Azure Data Lake Store](/azure/data-lake-store/) ou o [Azure Cosmos DB](/azure/cosmos-db/introduction). Você pode executar a análise em lotes em saídas armazenadas com o Azure HDInsight ou enviar a saída para outro serviço, como os Hubs de Eventos para consumo ou para o [Power BI](https://docs.microsoft.com/power-bi/) para visualização em tempo real.
...
Como um serviço gerenciado, o Stream Analytics garante o processamento de eventos com 99,9% de disponibilidade no nível do minuto. Para saber mais, confira a página do [SLA do Stream Analytics](https://azure.microsoft.com/support/legal/sla/stream-analytics/v1_0/). 
