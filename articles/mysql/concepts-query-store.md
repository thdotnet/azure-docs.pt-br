---
title: Store consulta no banco de dados do Azure para MySQL
description: Este artigo descreve o recurso de Store de consulta no banco de dados do Azure para MySQL
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
1. Entre portal do Azure e selecione seu banco de dados do Azure para servidor MySQL.
...
Use o [portal do Azure](howto-server-parameters.md) ou [da CLI do Azure](howto-configure-server-parameters-using-cli.md) para obter ou definir um valor diferente para um parâmetro.

## <a name="views-and-functions"></a>Exibições e funções

Exiba e gerencie o Repositório de Consultas usando as seguintes exibições e funções. Qualquer pessoa a [função pública de privilégio selecione](howto-create-users.md#how-to-create-additional-admin-users-in-azure-database-for-mysql) pode usar esses modos de exibição para ver os dados na consulta Store. Essas exibições estão disponíveis somente na **mysql** banco de dados.
...
- Se um servidor MySQL tem o parâmetro `default_transaction_read_only` , consulta Store não é possível capturar dados.
