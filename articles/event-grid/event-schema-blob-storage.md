...
> Esses eventos estão em visualização pública e estão disponíveis apenas nas regiões **oeste dos EUA 2** e **Oeste EUA Central** .
 ...
 |**Microsoft.Storage.BlobDeleted** |Disparado quando um blob é excluído. <br>Especificamente, esse evento também é disparado quando os clientes chamam a operação `DeleteFile` que está disponível na API REST do Azure Data Lake Storage Gen2. |
...
> [!NOTE]
> Se você quiser garantir que o evento **Microsoft. Storage. BlobCreated** seja disparado somente quando um blob de blocos for completamente confirmado, filtre o evento para a chamada à API REST `FlushWithClose`. Essa chamada à API dispara o evento **Microsoft. Storage. BlobCreated** somente depois que os dados são totalmente confirmados em um blob de blocos. Para saber como criar um filtro, consulte [filtrar eventos para a grade de eventos](https://docs.microsoft.com/azure/event-grid/how-to-filter-events).
...
* A chave `dataVersion` é definida com um valor de `2`.

* A chave `data.api` é definida como a cadeia de caracteres `CreateFile` ou `FlushWithClose`.
...
* A chave `data.api` é definida como a cadeia de caracteres `DeleteFile`.
...
| url | string | O caminho para o blob. <br>Se o cliente usar uma API REST de BLOB, a URL terá essa estrutura: *@no__t -1storage-Account-Name\>.blob.core.windows.net/\<container-Name @ no__t-4 @ no__t-5 @ no__t-6file-Name @ no__t-7*. <br>Se o cliente usar uma API REST Data Lake Storage, a URL terá essa estrutura: *@no__t -1storage-Account-Name\>.dfs.core.windows.net/\<file-System-Name @ no__t-4 @ no__t-5 @ no__t-6file-Name @ no__t-7*. |
