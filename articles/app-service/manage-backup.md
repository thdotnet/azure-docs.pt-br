...
Às vezes, você não quer fazer backup de tudo em seu aplicativo. Veja aqui alguns exemplos:
...
Carregue o arquivo `_backup.filter` no diretório `D:\home\site\wwwroot\` de seu site usando o [ftp](deploy-ftp.md) ou qualquer outro método. Se quiser, você pode criar o arquivo diretamente usando Kudu `DebugConsole` e inserir o conteúdo nesse local.
