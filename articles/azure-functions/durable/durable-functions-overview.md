...
Usando uma [*Durable Entity*](durable-functions-preview.md#entity-functions), é possível implementar esse padrão com facilidade como uma única função.
...
Os clientes podem enfileirar *operações* de uma *durable entity* (também conhecido como "sinalização") usando a [associação do cliente de entidade](durable-functions-bindings.md#entity-client).
