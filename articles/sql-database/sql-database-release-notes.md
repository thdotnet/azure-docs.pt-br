...
| Recurso | Detalhes |
| ---| --- |
| Recuperação de banco de dados acelerada com bancos de dados únicos e pools Elásticos | Para obter informações, consulte [recuperação de banco de dados acelerada](sql-database-accelerated-database-recovery.md).|
...
| Recriar bancos de dados descartados com instâncias gerenciadas |Para obter informações, consulte [recriar bancos de dados descartados na instância gerenciada do SQL](https://medium.com/azure-sqldb-managed-instance/re-create-dropped-databases-in-azure-sql-managed-instance-dc369ed60266).|
...
| --- | --- |
| Docs de camada de serviço em hiperescala atualizados para a versão GA| Para obter mais informações, consulte [camada de serviço em hiperescala para até 100 TB](sql-database-service-tier-hyperscale.md).|
|Docs de camada de computação sem servidor lançados com a visualização pública| Para obter mais informações, consulte [banco de dados SQL sem servidor (visualização)](sql-database-serverless.md).|
| &nbsp; |
...
| Melhorias nos serviços | Detalhes |
| --- | --- |
| Pontos de extremidade públicos para a instância gerenciada em visualização pública| Para obter mais informações, consulte [usando o Azure SQL Database managed instância forma segura com o ponto de extremidade público](sql-database-managed-instance-public-endpoint-securely.md).|
| Suporte de fuso horário para a instância em visualização pública gerenciada| Para obter mais informações, consulte [fuso horário no banco de dados de instância gerenciada do SQL](sql-database-managed-instance-timezone.md).|
| A proteção do Azure bancos de dados SQL com identidades gerenciadas na segunda visualização pública de versão| Ver [protegendo Azure bancos de dados SQL com identidades gerenciadas ficou mais fácil](https://azure.microsoft.com/blog/securing-azure-sql-databases-with-managed-identities-just-got-easier/).|
| &nbsp; |
...
| Pontos de extremidade públicos para a instância gerenciada em visualização pública| Para obter mais informações, consulte [usando o Azure SQL Database managed instância forma segura com o ponto de extremidade público](sql-database-managed-instance-public-endpoint-securely.md).|
| Suporte de fuso horário para a instância em visualização pública gerenciada| Para obter mais informações, consulte [fuso horário no banco de dados de instância gerenciada do SQL](sql-database-managed-instance-timezone.md). |
| Governança de recursos no banco de dados SQL | Para obter mais informações, consulte [governança de recursos no Azure SQL Database](https://azure.microsoft.com/blog/resource-governance-in-azure-sql-database/). || &nbsp; |
...
| Melhorias nos serviços | Detalhes |
| --- | --- |
| Suporte de escalabilidade horizontal para o banco de dados SQL do Azure lançados para disponibilidade geral de leitura | Para obter mais informações, consulte [expansão de leitura](sql-database-read-scale-out.md).|
| &nbsp; |
...
|Banco de dados SQL como uma fonte de dados de referência para o Stream Analytics. | Para obter mais informações, consulte [Stream Analytics](https://azure.microsoft.com/services/stream-analytics/) e [do Azure Stream Analytics agora dá suporte ao banco de dados SQL como entrada de dados de referência](https://azure.microsoft.com/blog/azure-stream-analytics-now-supports-azure-sql-database-as-reference-data-input/).|
...
| Conteúdo atualizado para a automação de trabalho usando scripts Transact-SQL | Atualizado e esclareceu o conteúdo para o uso [automação de trabalho usando scripts Transact-SQL](sql-database-job-automation-overview.md) para instâncias gerenciadas, pools Elásticos e bancos de dados individuais. |
