...
## <a name="incorrect-overage-invoice"></a>Fatura de excedente incorreta

Se acreditar que tem uma fatura de excedente incorreta, confira [Entender a fatura do Contrato Enterprise do Azure](billing-understand-your-bill-ea.md) para ajudar a reconciliar as cobranças de uso do Azure. Se acreditar que há um erro no excedente cobrado pela Microsoft, contate o suporte.

## <a name="change-a-po-number-for-an-overage-invoice"></a>Alterar o número da OC de uma fatura de excedente

O Portal do EA do Azure gera automaticamente uma PO (ordem de compra) padrão, a menos que o administrador do EA configure uma antes da data de faturamento. Um administrador do EA pode atualizar o número da OC no Portal do EA do Azure entrando e navegando até **Relatórios**. Um administrador do EA pode atualizar o número da OC. Na seção superior à direita, que mostra o número da OC, clique no símbolo de lápis e atualize as informações.

Um administrador do EA pode atualizar o número da OC durante o trimestre de faturamento atual antes da criação da fatura. Ele também pode atualizar o número da OC até sete dias após receber um email de notificação automático com a fatura.

Se o administrador do EA não atualizar o número da OC e for necessário um número novo, a única opção será concluir o crédito e refaturar. Se o cliente precisar atualizar o número da OC, contate seu parceiro para obter assistência.
...
_Encargos Cobrados Separadamente_ e _Serviços do Azure Marketplace_ são serviços terceirizados implantados da plataforma Azure Marketplace. A principal diferença entre os dois serviços é como eles são faturados, com base na estrutura contratual estabelecida com os fornecedores terceiros.

O uso de terceiros aparece na fatura em **Encargos Cobrados Separadamente**. O uso consome o saldo do compromisso monetário do Azure. Para exibir uma lista de serviços de terceiros, confira [Serviços de revendedores terceiros no Azure Marketplace agora usam compromisso monetário do Azure](https://azure.microsoft.com/updates/azure-marketplace-third-party-reseller-services-now-use-azure-monetary-commitment/).

Na fatura, o uso que aparece em **Azure Marketplace** é cobrado separadamente. Não é coberto por qualquer compromisso monetário. No entanto, todas as cobranças, incluindo Marketplace, excedente e encargos cobrados separadamente, são consolidados em uma única fatura.

Você recebe uma única fatura pelo excedente interno do Azure e pelas compras de terceiros no Marketplace do Azure.

Quando o compromisso monetário for totalmente consumido, o uso será cobrado mensalmente ou trimestralmente. Se o registro tiver sido iniciado antes de maio de 2018, uma regra de limite de compromisso monetário de 150% será aplicada e você será cobrado pelos excedentes trimestralmente. Se o registro tiver sido iniciado após de maio de 2018, todos os excedentes serão cobrados mensalmente.
