...
Quando você usa a exibição de gráfico de previsão, pode identificar possíveis violações de orçamento. Quando há uma possível violação de orçamento, o excesso de gastos projetados é mostrado em vermelho. Um símbolo indicador também é mostrado no gráfico. Focalizar o símbolo mostra a data estimada da violação de orçamento.
...
Por padrão, a análise de custo mostra todos os custos de uso e compra conforme eles são acumulados e serão exibidos na sua fatura, também conhecido como **Custo real**. Exibir o custo real é ideal para reconciliar sua fatura. No entanto, picos de compra no custo podem ser alarmantes quando você está atento a anomalias de gastos e outras alterações no custo. Para nivelar os picos causados pelos custos de compra de reserva, mude para **Custo amortizado**. 
...
| --- | --- |
| **Período de cobrança** | Detalhe os custos por mês da fatura. É importante que assinaturas de teste/desenvolvimento e pagas conforme o uso, que não estão associadas a meses do calendário. Contas EA/MCA podem usar meses do calendário no seletor de data e/ou a granularidade mensal para atingir a mesma meta. |
| **Tipo de preço** | Detalhe uso, compra, reembolso e custos de reserva não usada. Compras de reserva e reembolsos só estão disponíveis ao usar custos de ação, e não custos amortizados. Os custos de reserva não usada estão disponíveis somente ao examinar os custos amortizados. |
| **Nuvem** | Detalhe os custos por AWS e Azure. Os custos do AWS só estão disponíveis de grupos de gerenciamento, contas de cobrança externas e assinaturas externas. |
| **Departamento** / **Seção de fatura** | Detalhe os custos por seção de fatura MCA ou departamento EA. Disponível somente para contas de cobrança EA/MCA e perfis de cobrança MCA. |
| **Conta de registro** | Detalhe os custos pelo proprietário da conta EA. Disponível somente para departamentos e contas de cobrança EA. |
| **Frequência** | Detalhe os custos com base no uso, únicos e recorrentes. |
| **Medidor** | Detalhe o custo pelo medidor de uso do Azure. Disponível somente para uso do Azure. Todas as compras e uso do Marketplace mostrarão "Não especificado" ou "não atribuído". |
...
| **Recurso** | Detalhe os custos por recurso. Todas as compras serão mostradas como "Não especificado", pois elas são aplicadas no nível do perfil de cobrança MCA ou de conta de cobrança EA/PAYG.  |
| **Grupo de recursos** | Detalhe os custos por grupo de recursos. Disponível somente para uso não clássico. Uso de recurso clássico mostrará "outro" e compras mostrarão "Não especificado". |
| **Tipo de recurso** | Detalhe os custos por tipo de recurso. Disponível somente para uso não clássico. Uso de recurso clássico mostrará "outro" e compras mostrarão "Não especificado". |
| **Nome do serviço** ou **Categoria do medidor** | Detalhe o custo por serviço do Azure. Disponível somente para uso do Azure. Todas as compras e uso do Marketplace mostrarão "Não especificado" ou "não atribuído". |
| **Camada de serviço** ou **Subcategoria de medidor** | Detalhe o custo por subclassificação do medidor de uso do Azure. Disponível somente para uso do Azure. Todas as compras e uso do Marketplace mostrarão "Não especificado" ou "não atribuído". |
...
Para fixar a análise de custo, clique no ícone de alfinete no canto superior direito. Fixar a análise de custo salvará apenas a exibição principal de gráfico ou tabela, se estiver selecionada. Compartilhe o painel para dar a outras pessoas acesso ao bloco. Observe que isso apenas compartilha a configuração do painel e não permite acesso a outras pessoas aos dados subjacentes. Se alguém não tiver acesso aos custos e tiver acesso a um painel compartilhado, ele verá uma mensagem acesso negado.

Para compartilhar um link para a análise de custo, clique no comando **Compartilhar** na parte superior da folha. Uma URL personalizada será mostrada, abrindo essa exibição específica para esse escopo específico. Se alguém que não tem acesso de custo obtém a URL, essa pessoa verá uma mensagem de acesso negado. 
...
Às vezes você precisa baixar os dados para análise posterior, mesclá-lo com seus próprios dados ou integrá-los a seus próprios sistemas. O Gerenciamento de Custos oferece algumas opções aqui. Como um ponto de partida, se você precisar de um resumo ad-hoc de alto nível, como o que você obtém na análise de custo, crie a exibição de que você precisa e baixe-a clicando em **Exportar** e selecionando **Baixar os dados para CSV** ou **Baixar os dados para Excel**. O download para Excel fornece contexto adicional sobre a exibição que você usou para gerar o download, como escopo, configuração da consulta, total e data de geração.

Se você precisar do conjunto de dados completo não agregado, baixe-o da conta de cobrança. Na lista de serviços no painel de navegação esquerdo do portal, vá para Gerenciamento de Custos + Cobrança > (selecione sua conta de cobrança, se aplicável) > Uso + encargos, então clique no ícone de Download para o período de cobrança desejado.

Se você precisar automatizar a obtenção de dados de custo, terá uma abordagem semelhante: Use a [API de Consulta](/rest/api/cost-management/query) para uma análise mais avançada com filtragem dinâmica, agrupamento e agregação ou use a [API de UsageDetails API](/rest/api/consumption/usageDetails) para o conjunto de dados completo não agregado. A versão de GA dessas APIs é de 01/01/2019. Use a **2019-04-01-preview** para obter acesso à versão prévia de reserva e compras no Marketplace dentro dessas APIs. 
...
Se você precisar que os custos reais mostrem as compras conforme são acumuladas, altere **tipo**/**métrica** para **ActualCost**. Para obter mais informações sobre essas APIs, confira a documentação de API de [Query](/rest/api/cost-management/query) e [UsageDetails](/rest/api/consumption/usageDetails). Observe que os documentos publicados são para a versão de GA, mas ambos funcionam da mesma forma para a versão da API 2019-04-01-preview fora do novo atributo de tipo/métrica e os nomes de propriedade alterados. (Mais informações sobre os nomes de propriedade abaixo.)
 
As APIs de Gerenciamento de Custos funcionam em todos os escopos acima de recursos. Ou seja, grupo de recursos, assinatura e grupo de gerenciamento por meio do acesso de RBAC do Azure, contas de cobrança de EA (registros), departamentos e contas de registro por meio de acesso ao Portal do EA etc. Saiba mais sobre escopos, incluindo como determinar a ID do escopo e/ou gerenciar o acesso, em [Entender e trabalhar com escopos](understand-work-scopes.md).
