...
Como um administrador de TI, você pode usar [acesso condicional do Azure AD](../conditional-access/overview.md) para exigir que os usuários se autentiquem usando a autenticação multifator (MFA) ou conecte-se de um dispositivo ou rede confiável. Durante o planejamento de implantação, você descobre que alguns desses requisitos não podem ser atendidos por todos os usuários. Por exemplo, há usuários que trabalham em um escritório remoto que não faz parte de sua rede interna ou há um executivo que usa um telefone antigo que não é compatível. A empresa exige que esses usuários tenham permissão para entrar e fazer seu trabalho, portanto, elas são excluídas de políticas de acesso condicional.

Como outro exemplo, você pode usar [localizações nomeadas](../conditional-access/location-condition.md) no acesso condicional para configurar um conjunto de países e regiões do qual você não deseja permitir que usuários acessem seu locatário.
...
No entanto, se você configurar a exclusão para a política de acesso condicional usando um grupo do AD do Azure, em seguida, você pode usar as revisões de acesso como um controle de compensação, a visibilidade da unidade e reduzir o número de usuários que têm uma exceção.
...
Siga estas etapas para criar um novo grupo do Azure AD e uma política de acesso condicional não se aplica a esse grupo.
...
1. Selecione os usuários que devem fazer parte do grupo de exclusão e, em seguida, clique em **Criar**.
    ...
    ![Novo painel de grupo do Active Directory do Azure](./media/conditional-access-exclusion/new-group.png)
...
1. No painel de navegação esquerdo, clique em **Azure Active Directory** e, em seguida, clique em **acesso condicional** para abrir o **políticas** folha.
...
1. Continue com a configuração de política de acesso condicional com base nesses requisitos organizacionais.
...
Digamos que você tenha uma política de acesso condicional que bloqueia o acesso de usuários que usam versões mais antigas do cliente e autenticação herdados. Ela inclui um grupo que é excluído da política. Esta é uma revisão de acesso recomendada em que os membros do grupo são revisados.
...
Agora que tudo o que você tem em vigor, grupo, política de acesso condicional e as revisões de acesso, chegou a hora para monitorar e acompanhar os resultados dessas revisões.
...
1. Clique em **Resultados** para ver quem foi aprovado para permanecer na lista e quem foi removido.
    ...
    ![Revisões de acesso resultados mostram que tenha sido aprovada](./media/conditional-access-exclusion/access-reviews-results.png)

1. Em seguida, clique em **Logs de auditoria** para ver as ações que foram executadas durante a revisão.

    ![Os logs de auditoria listando ações de revisões de acesso](./media/conditional-access-exclusion/access-reviews-audit-logs.png)
...
- [O que é acesso condicional no Azure Active Directory?](../conditional-access/overview.md)
