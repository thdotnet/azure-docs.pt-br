...
1. Você precisa importar o Certificado de Metadados no F5 (Kerberos) que será usado posteriormente no processo de instalação. Acesse **Sistema > Gerenciamento de Certificado > Gerenciamento de Certificado de Tráfego >> Lista de Certificados SSL**. Clique em **Importar** do canto direito.

    ![Configuração do F5 (Kerberos)](./media/kerbf5-tutorial/configure01.png) 

1. Além disso, você também precisa de um **Certificado SSL** para o Nome do host (`Kerbapp.superdemo.live`); neste exemplo, usamos o Certificado Curinga.

    ![Configuração do F5 (Kerberos)](./media/kerbf5-tutorial/configure02.png) 
 
1. Acesse – **Acesso de Clique BIG-IP do F5 > Configuração Guiada > Federação > Provedor de Serviços SAML**.
...
1. Crie um Servidor Virtual e especifique o **Endereço de Destino**. Escolha o **Certificado Curinga** (ou o **Cert** que você carregou para o aplicativo) que carregamos anteriormente e a **Chave Privada Associada**.
...
1. Em **origem do nome de usuário**, especifique `session.saml.last.attr.name. http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname`. Confira no Apêndice uma lista completa de variáveis e valores.
...
1. Contate a [equipe de suporte ao Cliente do F5 (Kerberos)](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) para obter detalhes sobre a documentação de **Propriedades de Verificação do Ponto de Extremidade**.

    ![Configuração do F5 (Kerberos)](./media/kerbf5-tutorial/configure10.png) 

1. Contate a [equipe de suporte ao Cliente do F5 (Kerberos)](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) para obter detalhes sobre a documentação de **Propriedades de Gerenciamento de Sessão**.
